...
def train_baseline_model(
    ...
    try:
        ...
        logger.error(f"Error training {baseline_model.name_caption}: {e}", exc_info=True)
...
def train_model_with_config(
    ...
    config_param_grid = hp_config.get("param_grid") if isinstance(hp_config, dict) else None
    ...
    trainer = ModelTrainer(model=pipeline, task_type="classification", random_state=random_state)
    ...
    if tuning_performed:
        ...
        training_results = trainer.train(X_train, y_train, X_val, y_val, sample_weight=sample_weight)
