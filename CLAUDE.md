...
- **One ingested table.** `data/ingested/games_updated_history.csv` is the only
  game history. Exactly two steps write it: `ingest-raw-games` merges the raw
  archive in (the archive wins any shared gameId, except that a postponed archive
  row never supersedes a played one), and `append-game-results` adds the results
  inbox. Everything else reads it. A second "seed" table is what made
  `full-rebuild` silently discard the raw parse.
...
- **`data/raw/reference/` holds inputs, not artifacts.** Two lookups there
  (team locations, city distances) were generated by OpenAI/Serper and cannot be
  rebuilt offline — `parse_raw_games` fails at step one without them. Their
  builders are excluded from `full-rebuild` on purpose: a data rebuild must
  never depend on a paid API call that can return a worse table than the one it
  overwrites. See docs/PIPELINE_AUDIT.md.
- **`data/` is ignored except for four tracked inputs.** `.gitignore` un-ignores
  `data/raw/reference/` and `data/raw/historical/handmade/` — the only files no
  command can regenerate, and therefore the only ones a fresh clone cannot do
  without. Never write build output into those two directories: it would be
  committed by default. Everything else derived stays ignored.
- **Training does not deploy.** `make train` logs the run URI; only
  `PROMOTE=1` repoints `configs/predict/predict_classifier.yaml`.
...
Historical (make historical-etl):
  data/raw/historical/games/Games.csv
    → [ingest-raw-games]       → data/ingested/games_updated_history.csv
